...
export class HomeComponent {
  ...
  fileIdFromNotification = computed(() =>
    this.notifications().map((n) => n.fileId),
  );
  applicationName = computed(() =>
    this.notifications().map((n) => n.applicationName),
  );
  version = computed(() =>
    this.notifications().map(
      (n) => n.fileVersion || '2022-02-16T11:12:56.3052287Z',
    ),
  );
  ...
  constructor() {
    interval(2000)
      .pipe(
        switchMap(() => this.clientService.getNotifications()),
        catchError((error) => (this.file = error)),
      )
      .subscribe((result) =>
        this.notifications.set(result as NotificationModel[]),
      );
  ...
  public GetFile(notification: NotificationModel) {
    return this.clientService
      .GetFile(notification, this.key, this.askJson)
      .subscribe((result) =>
        this.file.set(
          this.IsJson(result) ? JSON.parse(result) : result.toString(),
        ),
      );
  }
  public saveKey() {
