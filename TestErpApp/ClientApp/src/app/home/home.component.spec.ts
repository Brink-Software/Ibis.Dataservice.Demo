...
describe('HomeComponent', () => {
	...
	it('should call GetFile and set file content', () => {
		const notification: NotificationModel = { fileId: '1', applicationName: 'app', fileVersion: 'v1', dataUrl: 'url' } as NotificationModel;
		component.GetFile(notification);
		expect(clientServiceMock.GetFile).toHaveBeenCalledWith(notification, 'test-key', false);
	});
	...
	it('should return notification identifier', () => {
		const notification: NotificationModel = { fileId: '1', applicationName: 'app', fileVersion: 'v1', dataUrl: 'url' } as NotificationModel;
		const identifier = component.notificationIdentifier(0, notification);
		expect(identifier).toBe('url');
	});
	...
	it('should handle dialog submit', () => {
		const event = { processedStatus: ProcessedStatus.Succeeded, comments: 'test' };
		const notification: NotificationModel = { fileId: '1', applicationName: 'app', fileVersion: 'v1', dataUrl: 'url' } as NotificationModel;
